...
- Decide whether the feature lives in an existing service or needs a new one. A new service is warranted when the feature owns aggregates no existing service touches and has its own pools, document collections, and deploy target. Creating one follows `docs/add-service.md`.
...
- Declare every new executor in `terraform.tfvars` with the `pool` it belongs to — that entry is its own command queue — and every new listener with its subscriptions.
