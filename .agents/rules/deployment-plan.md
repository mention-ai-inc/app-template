...
| a new `admin/admin/backfill/migrations/*.py` | a backfill run as a Container Apps job; see `deploy-branch` |
...
A deploy on Azure is `az containerapp update --image` against the freshly pushed image, which creates
a new revision and shifts all traffic to it, plus `az containerapp job update` for anything that runs
on a schedule. A job takes the new image and nothing else: its cron trigger already points at the job,
so the next run picks it up. Every Container App and job sets `ignore_changes` on the container's
image and command precisely so that a later `m terraform-services` does not roll a deployed revision
back.
...
`m terraform-services` is infrastructure the route depends on: a **new pool**, a **new Service Bus
queue** (that is, a new executor entry in `terraform.tfvars`), a **new topic subscription with its SQL
filter rule** (a new listener subscription), a **new Cosmos DB change feed trigger** (the lease
container the trigger checkpoints into), or a **new job**.

A pool's scale rules are Terraform's too. A Container App pool scales on the Service Bus queue or
subscription depth named in its `custom_scale_rule` blocks, so a route whose queue Terraform has not
created yet leaves the pool with nothing to scale on, and `SERVICE_BUS_QUEUES_JSON`,
`SERVICE_BUS_SUBSCRIPTIONS_JSON` and `CHANGE_FEED_TRIGGERS_JSON` are how the pool driver finds its
sources at all. A pool whose routing table names no source it serves fails loudly on start rather than
idling.
...
- Terraform that creates a topic subscription applies **after** the code that handles the event
  exists — the subscription accumulates matching messages from the moment it is created, and the pool
  must already serve the route or they exhaust `max_delivery_count` and land in the subscription's
