...
from tests.infrastructure.mocks import patch_firestore_transaction, patch_firestore_transactions
...
async def test_nested_contexts_restore_outer_uow_on_inner_exit(mocker: MockerFixture) -> None:
    ...
    async with unit_of_work():
        assert get_current_uow() is outer_transaction
        async with unit_of_work():
            assert get_current_uow() is inner_transaction
        assert get_current_uow() is outer_transaction
