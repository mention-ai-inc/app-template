...
- **Cross-service Pub/Sub contracts** — `domain/commands/*` and `domain/events/*`, plus any value object that is published as part of an event/command payload and consumed by more than one service.
...
- **Shared infrastructure framework** — the request/response and messaging plumbing every service is built on: `presentation/api`, `presentation/auth`, `application/{unit_of_work,users,events}`, `infrastructure/{firestore,repository,llm,cloud/*}`.
...
- **Commands and events** over Pub/Sub (`domain/commands/*`, `domain/events/*`), or
