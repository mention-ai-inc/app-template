...
Deploys run through GitHub Actions; **backfills do not** — there is no Backfill workflow, and on AWS
they run as ECS tasks against the deployed admin image (section 7). `gh` is required — no MCP tool can
dispatch a workflow. Preflight `gh auth status`; the token needs the `repo` scope, which covers both
dispatching workflows and reading runs. If it is invalid, stop and ask the user to run
`gh auth login -h github.com` (suggest they type `! gh auth login -h github.com`). Do not attempt to
work around a bad token.

You also need AWS credentials that can assume the `terraform` role. `aws sts get-caller-identity`
proves them; `AWS_PROFILE` comes from `infrastructure/cli/provider/envrc`. Everything CI does is
GitHub OIDC — there are no access keys anywhere, and you should never create one.
...
on AWS (see section 7), so there is no deployed source of truth to cross-check against.
...
Backfills (in order, run as an ECS task against `admin-j-backfill` by an engineer):
...
Then run the backfill as an ECS task, exactly as in section 7 but against the `demo`-prefixed family
(`demoadmin-j-backfill`). Dry-run first (drop `--apply`) and read the CloudWatch log stream it
...
deploy: images build into `<branch>services` and `<branch>admin` ECR repositories that do not exist,
while the terraform steps still apply against production. Pass `--ref main` explicitly.
...
every `m admin` subcommand fails on import before it reaches AWS. Do not try to make it work; run the
job directly.

**Run the backfill as an ECS task.** That is what the admin API would do once it had authenticated,
minus the audit event, and it runs as the caller's own AWS identity. The task definition family is
`admin-j-backfill` in production (`<env>admin-j-backfill` elsewhere); the network configuration must
match what Terraform gave the job:

```bash
CLUSTER=acme
SUBNETS=$(terraform -chdir=infrastructure/terraform/configurations/operations output -json private_subnet_ids | jq -r 'join(",")')
SECURITY_GROUP=$(aws ecs describe-task-definition --task-definition admin-j-backfill \
  --query 'tags' --output text)

aws ecs run-task --cluster "$CLUSTER" --launch-type FARGATE \
  --task-definition admin-j-backfill \
  --network-configuration "awsvpcConfiguration={subnets=[$SUBNETS],securityGroups=[$SECURITY_GROUP],assignPublicIp=DISABLED}" \
  --overrides '{"containerOverrides":[{"name":"app","command":["admin","backfill","run","<backfill-name>"]}]}' \
  --query 'tasks[0].taskArn' --output text
```

Take the security group from the job module's output (`m terraform-admin` prints it, or
`aws ec2 describe-security-groups --filters Name=group-name,Values=admin-j-backfill*`) — an ECS task
started without the job's own security group cannot reach DynamoDB's VPC endpoint or the cache.

The container command is the bare `admin backfill` CLI — `run <name> [--apply] [--organization-id <id>]`.
There is no `--confirm production`; that flag was a client-side guard in `m admin`. Dry run first,
read its output, then re-run with `--apply` appended — unless the user waives the dry run.
`run-task` returns as soon as the task is accepted, so wait for it and then read the log:

```bash
aws ecs wait tasks-stopped --cluster acme --tasks <task arn>
aws logs tail /ecs/admin-j-backfill --since 30m --format short
```

The task runs the **deployed** admin image, so admin must have deployed first — see the two-dispatch
...
too that step 1's terraform apply already destroys the SQS queue and the SNS subscriptions of any
...
there, but nothing can reach it. Messages still sitting in a destroyed queue go with it, and the old
code's attempts to dispatch that command fail loudly — `SQS_EXECUTOR_QUEUES_JSON` no longer names a
...
- MCP terraform applies only when `mcp=true`; admin terraform and the admin image deploy only when
  `admin=true`.
- `terraform_operations=true` applies operations terraform — shared infrastructure, and the account's
  only VPC, ECS cluster and hosted zone live in it. Confirm with the user before including it even if
  the diff implies it.
- There is no library checkbox. Library changes mean ticking every service explicitly.
- A deploy leaves the previous task definition revision in place. Rolling back is
  `aws ecs update-service --cluster acme --service <name> --task-definition <family>:<previous revision>`,
  which is faster than re-running the workflow on an older ref.
