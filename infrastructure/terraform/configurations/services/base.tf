...
locals {
  ...
  servicebus_namespace_id      = local.is_production ? local.operations.production_servicebus_namespace_id : local.operations.feature_servicebus_namespace_id
  ...
  servicebus_namespace_name    = local.is_production ? local.operations.production_servicebus_namespace_name : local.operations.feature_servicebus_namespace_name
  ...
  storage_account_id           = local.is_production ? local.operations.production_storage_account_id : local.operations.feature_storage_account_id
  ...
  dns_zone_id                  = local.operations.dns_zone_id

  front_door_profile_id    = local.is_production ? local.operations.production_front_door_profile_id : local.operations.feature_front_door_profile_id
  front_door_resource_guid = local.is_production ? local.operations.production_front_door_resource_guid : local.operations.feature_front_door_resource_guid

  redis_sku_name = local.is_production ? var.redis_sku_name : var.feature_redis_sku_name
  redis_family   = local.is_production ? var.redis_family : var.feature_redis_family
  redis_capacity = local.is_production ? var.redis_capacity : var.feature_redis_capacity

  image_repository = "${local.registry_login_server}/${local.feature_environment}services"
}
