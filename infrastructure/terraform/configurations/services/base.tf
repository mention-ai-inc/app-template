locals {
  ...
  state_bucket       = "acme-operations-0000--terraform-state"
}
...
provider "aws" {
  ...
  assume_role {
    ...
    session_name = "terraform-services"
  }
...
locals {
  ...
  cache_node_type     = local.is_production ? var.redis_node_type : var.feature_redis_node_type
  bucket_name_prefix  = data.terraform_remote_state.operations.outputs.bucket_name_prefix
}
