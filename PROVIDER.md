...
This branch is AWS's provider branch. **It is not implemented yet** — it is currently identical to
...
to behave differently on AWS, add a hook to the slot on `main` and put the behaviour in the slot —
...
- [ ] `infrastructure/cli/provider/feature-environment` — `$FEATURE_ENVIRONMENT` to an AWS account.
...
- [ ] `docs/bootstrap.md` — an empty AWS account to a first deploy.
- [ ] A root ignore file, if AWS's tooling needs one.
